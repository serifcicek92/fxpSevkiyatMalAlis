...
DEFINE CLASS sevkiyatmalalis  AS Session OLEPUBLIC
	HIDDEN BPATH
	HIDDEN STKPATH
	HIDDEN KLASOR
	HIDDEN _CEPNO
...
**Dönüþ Yap
	...
	PROCEDURE donusYapTest(plkNo)
	    ...
	    * Plaka bulunduðu sürece (WHILE plakano = plkNo) tüm kayýtlarý tara
	    IF FOUND()
	        SCAN WHILE plakano = plkNo
	            * Dönüþ tarihi boþ olan VE sevkiyat tarihi son 2 gün içinde olanlarý bul
	            IF donustarih < CTOD("1/1/1") .AND. sevktarihi > DATE() - 2
	                REPLACE donustarih WITH DATE(), donussaati WITH TIME(1)
	                lIslemYapildi = .T. && En az 1 kayýt güncellendiðini iþaretle
	            ENDIF
	        ENDSCAN
	    ENDIF

	    * Eðer döngü içinde en az 1 kayýt kapatýldýysa true dön
	    IF lIslemYapildi = .T.
	        RETURN "true" 
	    ELSE 
	        RETURN "false"
...
*Plaka Sevk Saati Ekle
	PROCEDURE plakaSevkSaatiEkle(plkNo)
		...
		**LOCATE FOR plakano=plkNo .and. donustarih<CTOD("1/1/1") .and. sevktarihi>DATE()-2
		...
		IF FOUND() then
			LOCATE REST WHILE plakano = plkNo FOR donustarih < CTOD("1/1/1") .AND. sevktarihi > DATE()-1
	...
	*--------------------------------------------------------------------------------------------------
	...
	PROCEDURE UseTables()
		...
		* 1. Sevklist
		IF !USED("sevklist")
			USE (STKPATH + "sevklist") IN 0 SHARED ALIAS sevklist
		ENDIF
		
		* 2. Araclar
		IF !USED("araclar")
			USE (STKPATH + "araclar") IN 0 SHARED ALIAS araclar
			SELECT araclar
			SET ORDER TO 1
		ENDIF
		...
		* 3. Surucu
		IF !USED("surucu")
			USE (STKPATH + "surucu") IN 0 SHARED ALIAS surucu
			SELECT surucu
			SET ORDER TO 1
		ENDIF
		...
		* 4. Ftrrpr
		IF !USED("ftrrpr")
			USE (STKPATH + "ftrrpr") IN 0 SHARED ALIAS ftrrpr
			SELECT ftrrpr
			SET ORDER TO 1
		ENDIF
		...
		* 5. Ftrcikis
		IF !USED("ftrcikis")
			USE (STKPATH + "ftrcikis") IN 0 SHARED ALIAS ftrcikis
		ENDIF
	ENDPROC
	
	*--------------------------------------------------------------------------------------------------
	PROCEDURE UseTableseski()
		this.definePaths()
		STKPATH = this.STKPATH 
	...
	*Eczane Sevk Kayýt Ýþlemleri
	PROCEDURE EczaneCikisSevkKayit(eczkod,tkpno,fatno,sevkno,plkno,ss,ps,bs,ks)	
		...
		IF FOUND() then
			...
			IF !FOUND()
				APPEND BLANK
				REPLACE NEXT 1 faturano WITH fatno, eczanekodu WITH ftrrpr->eczanekodu, tarih WITH ftrrpr->tarih
			ENDIF 
			REPLACE NEXT 1 sevklistno WITH ftrrpr->sevklistno, sevktarihi WITH DATE(), sevksaati WITH TIME(1),;
	...
	** Son faturalarý, þifreli saatleri çözerek ve baðlý sepetlerle birlikte getirir
	PROCEDURE getSonSevkiyatDetayli(pGeriyeBak)
		**logYaz("sevklog.txt", " METODA GÝRDÝ")
	    this.definePaths()
	    STKPATH = this.STKPATH
	    ...
	    ** Tablolary aç
	    ...
	    ENDIF
	    **logYaz("sevklog.txt", TTOC(DATETIME()) + " SEPET AÇILDI" + CHR(13)+CHR(10))
	    USE &STKPATH.ftrrpr SHARED IN 0
	    **logYaz("sevklog.txt", TTOC(DATETIME()) + " FTRRPR AÇILDI - RECCOUNT:" + TRANSFORM(RECCOUNT()) + CHR(13)+CHR(10))    
	    SELECT ftrrpr
	    SET ORDER TO 0  
	    ...
	    ** Son kayytlara zypla
	    ...
	    JSONMetin = "["
	    **logYaz("sevklog.txt", TTOC(DATETIME()) + " SCAN BAÞLIYOR" + CHR(13)+CHR(10))
	    cCepNo = ALLTRIM(TRANSFORM(this._CEPNO))
	    SCAN REST FOR ALLTRIM(TRANSFORM(ftrrpr.cepno)) == cCepNo 
	    	**logYaz("sevklog.txt", TTOC(DATETIME()) + " SCAN ÝÇÝ - faturano:" + TRANSFORM(ftrrpr.faturano) + CHR(13)+CHR(10))
	        ...
	        ** Sepeti olmayan faturayy es geç 
	        IF EMPTY(sepetstr)
	            SELECT ftrrpr
	            LOOP
	        ENDIF
	        ...
	        ** Olasy enter karakterlerini temizle ve ana metne ekle
